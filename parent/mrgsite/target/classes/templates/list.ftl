		...
		$(".auditClass").click(function(){
			...
			$.ajax({
				...
				success:function(data){
                    $("#editForm_id").val(data.id);
                    $("#applier").text(data.ap_username);
                    $("#fileType").text(data.filetype.title);
				...
				<div class="panel panel-default">
					<table class="table">
						...
						<#list pageResult.listData as info>
							<tr>
								<td>${info.ap_username}</td>
								<td>${info.stateDisplay}</td>
								<td>${(info.score)!''}</td>
								<td>${(info.filetype.title)!""}</td>
								<td>${(info.au_username)!""}</td>
								<td>
									<#if (info.state == 0)>
									<a href="javascript:void(-1);" class="auditClass" data-uid='${info.id}'>审核</a>
									</#if>
								</td>
							</tr>
						</#list>
		      ...
		      <div class="modal-body">
		      	<form class="form-horizontal" id="editform" method="post" action="/userFile_audit">
					<fieldset>
						...
						<input type="hidden" name="state" id="stateId" value="" />
						<div class="form-group">
